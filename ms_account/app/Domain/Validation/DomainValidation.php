...
class DomainValidation
{
    ...
    public static function strNotNullAndMinAndMaxLength(string $value = '', int $minLenght = 2, int $length = 255, ?string $exceptMessage = null)
    {
        if (empty($value) && strlen($value) < $minLenght && strlen($value) > $length) {
            throw new EntityValidationException($exceptMessage ?? "The value must not be greater than {$length} characters");
        }
    }
}
